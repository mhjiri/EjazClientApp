...
export default observer(function CategoryGrid({className} : Props) {
  const { categoryStore } = useStore();
  const { activateCategory, deactivateCategory, loadCategories, setPagingParams, setOrderParams, setFilterParams, pagination, returnedCategories, axiosParams } = categoryStore;
  ...
  function handleGetNext() {
    ...
    loadCategories().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateCategory(event.target.id);
    else deactivateCategory(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/categories/add' className='btn btn-primary align-self-center'>
            Add Category
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {categoryStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedCategories && returnedCategories.map(category => (
                        <tr key={category.ct_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${category.ct_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {category.ct_Title}
                                </a>
                              </div>
                            </div>
                          ...
                          <td className='min-w-100px text-center'>
                            <span className='text-dark text-center fw-bold text-hover-primary d-block fs-6'>
                              {category.ct_Summaries}
                            </span>
                          </td>
                          
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={category.ct_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={category.ct_Active} value='Active'>Active</option>
                                <option selected={!category.ct_Active} value='Disbale'>Disable</option>

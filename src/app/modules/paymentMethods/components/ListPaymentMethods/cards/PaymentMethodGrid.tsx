...
export default observer(function PaymentMethodGrid({className} : Props) {
  const { paymentMethodStore } = useStore();
  const { activatePaymentMethod, deactivatePaymentMethod, loadPaymentMethods, setPagingParams, setOrderParams, setFilterParams, pagination, returnedPaymentMethods, axiosParams } = paymentMethodStore;
  ...
  function handleGetNext() {
    ...
    loadPaymentMethods().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activatePaymentMethod(event.target.id);
    else deactivatePaymentMethod(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/paymentMethods/add' className='btn btn-primary align-self-center'>
            Add PaymentMethod
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {paymentMethodStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedPaymentMethods && returnedPaymentMethods.map(paymentMethod => (
                        <tr key={paymentMethod.py_ID}>
                          <td>
                            <div className='form-check form-check-sm form-check-custom form-check-solid'>
                          ...
                          <td className='min-w-200px'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${paymentMethod.py_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {paymentMethod.py_Title}
                                </a>
                              </div>
                            </div>
                          </td>
                          <td className='min-w-100px text-end'>
                            <a href={`./view/${paymentMethod.py_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                              {paymentMethod.py_Title_Ar}
                            </a>
                          </td>
                          
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={paymentMethod.py_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={paymentMethod.py_Active} value='Active'>Active</option>
                                <option selected={!paymentMethod.py_Active} value='Disbale'>Disable</option>
                          ...
                          <div className='d-flex flex-shrink-0 justify-content-end'>
                              <a
                                href={`./view/${paymentMethod.py_ID}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >
                                <KTIcon iconName='magnifier' className='fs-3' />
                              </a>
                              <a
                                href={`./update/${paymentMethod.py_ID}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >

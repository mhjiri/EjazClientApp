...
export default observer(function TagGrid({className} : Props) {
  const { tagStore } = useStore();
  const { activateTag, deactivateTag, loadTags, setPagingParams, setOrderParams, setFilterParams, pagination, returnedTags, axiosParams } = tagStore;
  ...
  function handleGetNext() {
    ...
    loadTags().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateTag(event.target.id);
    else deactivateTag(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/tags/add' className='btn btn-primary align-self-center'>
            Add Tag
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {tagStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedTags && returnedTags.map(tag => (
                        <tr key={tag.tg_ID}>
                          <td>
                            <div className='form-check form-check-sm form-check-custom form-check-solid'>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${tag.tg_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {tag.tg_Title}
                                </a>
                              </div>
                            </div>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              ...
                              >
                                <option selected={tag.tg_Active} value='Active'>Active</option>
                                <option selected={!tag.tg_Active} value='Disbale'>Disable</option>

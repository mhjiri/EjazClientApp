...
export default observer(function GenreGrid({className} : Props) {
  const { genreStore } = useStore();
  const { activateGenre, deactivateGenre, loadGenres, setPagingParams, setOrderParams, setFilterParams, pagination, returnedGenres, axiosParams } = genreStore;
  ...
  function handleGetNext() {
    ...
    loadGenres().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateGenre(event.target.id);
    else deactivateGenre(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/genres/add' className='btn btn-primary align-self-center'>
            Add Genre
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {genreStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedGenres && returnedGenres.map(genre => (
                        <tr key={genre.gn_ID}>
                          <td>
                            <div className='form-check form-check-sm form-check-custom form-check-solid'>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${genre.gn_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {genre.gn_Title}
                                </a>
                              </div>
                            </div>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={genre.gn_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={genre.gn_Active} value='Active'>Active</option>
                                <option selected={!genre.gn_Active} value='Disbale'>Disable</option>

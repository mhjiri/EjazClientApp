...
export default observer(function AvatarGrid({className} : Props) {
  const { avatarStore } = useStore();
  const { activateAvatar, deactivateAvatar, loadAvatars, setPagingParams, setOrderParams, setFilterParams, pagination, returnedAvatars, axiosParams } = avatarStore;
  ...
  function handleGetNext() {
    ...
    loadAvatars().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateAvatar(event.target.id);
    else deactivateAvatar(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/avatars/add' className='btn btn-primary align-self-center'>
            Add Avatar
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {avatarStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedAvatars && returnedAvatars.map(avatar => (
                        <tr key={avatar.av_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${avatar.av_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {avatar.av_Title}
                                </a>
                              </div>
                            </div>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={avatar.av_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={avatar.av_Active} value='Active'>Active</option>
                                <option selected={!avatar.av_Active} value='Disbale'>Disable</option>

...
export default observer(function BookGrid({className} : Props) {
  const { bookStore } = useStore();
  const { activateBook, deactivateBook, loadBooks, setPagingParams, setOrderParams, setFilterParams, pagination, returnedBooks, axiosParams } = bookStore;
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateBook(event.target.id);
    else deactivateBook(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/books/add' className='btn btn-primary align-self-center'>
            Add Book
          </Link>
        </div>
      </div>
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      <thead>
                        <tr className='fw-bold text-muted'>
                          ...
                          <th id='language' className={axiosParams.get('orderBy')=='language' && axiosParams.get('orderAs')=='asc' ? 'min-w-100px text-center table-sort-asc' : axiosParams.get('orderBy')=='summeries' ? 'min-w-100px text-center table-sort-desc' : 'min-w-100px text-center'}>
                            <Link to='#' onClick={() => handleSortGrid('language')}>
                              Language
                            </Link>
                          </th>
                        ...
                        <tr key={book.bk_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column text-start'>
                                <a href={`./view/${book.bk_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {book.bk_Name}
                                </a>
                              </div>
                            </div>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={book.bk_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={book.bk_Active} value='Active'>Active</option>
                                <option selected={!book.bk_Active} value='Disbale'>Disable</option>

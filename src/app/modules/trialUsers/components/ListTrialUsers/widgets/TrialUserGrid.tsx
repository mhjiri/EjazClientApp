...
export default observer(function TrialUserGrid({className} : Props) {
  const { trialUserStore } = useStore();
  const { activateTrialUser, deactivateTrialUser, loadTrialUsers, setPagingParams, setOrderParams, setFilterParams, pagination, returnedTrialUsers, axiosParams } = trialUserStore;
  ...
  function handleGetNext() {
    ...
    loadTrialUsers().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateTrialUser(event.target.id);
    else deactivateTrialUser(event.target.id);
    ...
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        <div
          ...
          title='Click to add trialUser'
        >
          <Link to='/trialUsers/add' className='btn btn-primary align-self-center'>
            Add TrialUser
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {trialUserStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedTrialUsers && returnedTrialUsers.map(trialUser => (
                        <tr key={trialUser.username}>
                          ...
                          <td className='min-w-200px'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${trialUser.username}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {trialUser.us_DisplayName}
                                </a>
                              </div>
                            </div>
                          </td>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={trialUser.username}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={trialUser.us_Active} value='Active'>Active</option>
                                <option selected={!trialUser.us_Active} value='Disbale'>Disable</option>
                          ...
                          <div className='d-flex flex-shrink-0 justify-content-end'>
                              <a
                                href={`./view/${trialUser.username}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >
                                <KTIcon iconName='magnifier' className='fs-3' />
                              </a>
                              <a
                                href={`./update/${trialUser.username}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >

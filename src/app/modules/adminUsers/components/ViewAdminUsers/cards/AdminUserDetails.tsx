...
export default observer(function AdminUserDetails({id}: Props) {
  const { adminUserStore, genreStore, mediumStore } = useStore();
  const { loadAdminUser, deleteAdminUser, selectedAdminUser, loadingInitial } = adminUserStore;
  ...
  useEffect(() => {
    ...
    if (id) 
    { 
      loadAdminUser(id).then((adminUser: AdminUserFormValues | undefined) => setAdminUser(new AdminUserFormValues(adminUser)));
    } 
  }, [id, loadAdminUser])
  ...
  function handleDelete() {
    setConfirm(0)
    if(id) deleteAdminUser(id).then(() => navigate(`/adminUsers/list/`))
  }
        ...
        <div className='card mb-5 mb-xl-10'>
          ...
          <div id='adminUser_details' className='collapse show'>
            ...
            {(confirm == 1) ? (
              ...
              <Link to='/adminUsers/list' className='btn btn-light align-self-center mx-10'>
                List
              </Link>
              <Link to={`/adminUsers/update/${adminUser.username}`} className='btn btn-primary align-self-center'>
                Edit
              </Link>
            </div>
          </div>

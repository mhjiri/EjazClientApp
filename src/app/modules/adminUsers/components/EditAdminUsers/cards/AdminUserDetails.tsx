...
export default observer(function AdminUserDetails({id}: Props) {
  ...
  const { updateAdminUser, selectedAdminUser, loadAdminUser, uploadMedium, deleteMedium, setSrc, loadingInitial, uploading, uploaded, src } = adminUserStore;
  ...
  function handleFormSubmit(adminUser: AdminUserFormValues) {
    updateAdminUser(adminUser).then(() => navigate(`/adminUsers/view/${adminUser.username}`))
  }

...
export default observer(function BookCollectionGrid({className} : Props) {
  const { bookCollectionStore } = useStore();
  const { activateBookCollection, deactivateBookCollection, loadBookCollections, setPagingParams, setOrderParams, setFilterParams, pagination, returnedBookCollections, axiosParams } = bookCollectionStore;
  ...
  function handleGetNext() {
    ...
    loadBookCollections().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateBookCollection(event.target.id);
    else deactivateBookCollection(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/bookCollections/add' className='btn btn-primary align-self-center'>
            Add Book Collection
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {bookCollectionStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedBookCollections && returnedBookCollections.map(bookCollection => (
                        <tr key={bookCollection.bc_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${bookCollection.bc_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {bookCollection.bc_Title}
                                </a>
                              </div>
                            </div>
                          </td>
                          <td className='min-w-100px text-end'>
                            <a href={`./view/${bookCollection.bc_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                              {bookCollection.bc_Title_Ar}
                            </a>
                          </td>
                          
                          <td className='min-w-100px text-center'>
                            <span className='text-dark text-center fw-bold text-hover-primary d-block fs-6'>
                              {bookCollection.bc_Summaries}
                            </span>
                          </td>
                          
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={bookCollection.bc_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={bookCollection.bc_Active} value='Active'>Active</option>
                                <option selected={!bookCollection.bc_Active} value='Disbale'>Disable</option>
                          ...
                          <div className='d-flex flex-shrink-0 justify-content-end'>
                              <a
                                href={`./view/${bookCollection.bc_ID}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >
                                <KTIcon iconName='magnifier' className='fs-3' />
                              </a>
                              <a
                                href={`./update/${bookCollection.bc_ID}`}
                                className='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1'
                              >

...
export default observer(function GroupGrid({className} : Props) {
  const { groupStore } = useStore();
  const { activateGroup, deactivateGroup, loadGroups, setPagingParams, setOrderParams, setFilterParams, pagination, returnedGroups, axiosParams } = groupStore;
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activateGroup(event.target.id);
    else deactivateGroup(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/groups/add' className='btn btn-primary align-self-center'>
            Add Group
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {groupStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                        ...
                        <tr key={group.gr_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${group.gr_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {group.gr_Title}
                                </a>
                              </div>
                            </div>
                          ...
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={group.gr_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={group.gr_Active} value='Active'>Active</option>
                                <option selected={!group.gr_Active} value='Disbale'>Disable</option>

...
export default observer(function PublisherGrid({className} : Props) {
  const { publisherStore } = useStore();
  const { activatePublisher, deactivatePublisher, loadPublishers, setPagingParams, setOrderParams, setFilterParams, pagination, returnedPublishers, axiosParams } = publisherStore;
  ...
  function handleGetNext() {
    ...
    loadPublishers().then(() => setLoadingNext(false));
  }
  ...
  const handleChange = (event:any) => {
    ...
    if(event.target.value == 'Active') activatePublisher(event.target.id);
    else deactivatePublisher(event.target.id);
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        ...
        >
          <Link to='/publishers/add' className='btn btn-primary align-self-center'>
            Add Publisher
          </Link>
        </div>
      </div>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
              ...
              {publisherStore.loadingInitial && !loadingNext ? (
                <>
                  <LoadingComponent />
                </>
              ) : (
                <InfiniteScroll 
                  ...
                  >
                    ...
                    <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 thead-dark'>
                      ...
                      {/* begin::Table body */}
                      <tbody>
                      {(returnedPublishers && returnedPublishers.map(publisher => (
                        <tr key={publisher.pb_ID}>
                          ...
                          <td className='min-w-200px text-end'>
                            <div className='d-flex align-items-center'>
                              <div className='d-flex justify-content-start flex-column'>
                                <a href={`./view/${publisher.pb_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                                  {publisher.pb_Name}
                                </a>
                              </div>
                            </div>
                          ...
                          <td className='min-w-100px text-end'>
                            <a href={`./view/${publisher.pb_ID}`} className='text-dark fw-bold text-hover-primary fs-6'>
                              {publisher.pb_Name_Ar}
                            </a>
                          </td>
                          
                          <td className='min-w-100px text-center'>
                            <span className='text-dark text-center fw-bold text-hover-primary d-block fs-6'>
                              {publisher.pb_Summaries}
                            </span>
                          </td>
                          
                          <td  className='min-w-100px text-end'>
                            <div className='d-flex lex-shrink-0'>
                              <select id={publisher.pb_ID}
                              className='form-select form-select-solid form-select-sm'
                              onChange={handleChange}
                              >
                                <option selected={publisher.pb_Active} value='Active'>Active</option>
                                <option selected={!publisher.pb_Active} value='Disbale'>Disable</option>
